...
def saveP():
    for item in tree.get_children():
        ...
        file2B = os.path.join(var2.get(), "saveP",
                              'fand_' + item_text[2] + item_text[3])  #复制去的文件
...
def sieve():  #筛选函数
    #if 各个值不为空，继续
    if var7.get() == '' or var8_1.get() == '' or var8_2.get(
    ) == '' or var9_1.get() == '' or var9_2.get() == '':
        ...
        for item_textS in listS:
            tree.insert('',
                        END,
                        values=[
                            item_textS[0], item_textS[1], item_textS[2],
                            item_textS[3], item_textS[4], item_textS[5],
                            item_textS[6], item_textS[7]
                        ])
...
def recommend(path, path2):  #根据图片路径的推荐函数
    ...
    tabControl.select(2)
    label3.config(text='正在生成推荐，请勿操作！！')
    root.update()  #跳转推荐页（序号2的tab）
    a1, a2 = recommendSingl.single(path)[0], recommendSingl.single(path)[
        1]  #尽量减少运行次数，太慢了/a1是文字、a2是id
    x1.set(str(a2) + a1)
    root.update()
    list = recommendAll.fandSame(a2, path2)  #在范围内搜索吧
    lbExecute(list)  #清空再根据列表输出lb
    showPic3(list[0])  #显示第一张
    label3.config(text='推荐已完成')
    root.update()
...
def treeviewClick(event):  #单击展示选定的图片
    ...
    if var4.get() == 1:
        try:  #防止选中的没东西时报错
            recommend(item_text[1] + '\\' + item_text[2] + item_text[3],
                      var2.get())  #根据图片,产出标签推荐
        except:
            pass
...
def treeview_sort_column1(tv, col, reverse):  #Treeview、列名、排列方式（桶排序）
    ...
    for index, (val, k) in enumerate(l):
        tv.move(k, '', index)
    tv.heading(col,
               command=lambda: treeview_sort_column1(tv, col, not reverse))
...
def treeview_sort_column2(tv, col, reverse):  #Treeview、列名、排列方式（数字排序）
    ...
    for index, (val, k) in enumerate(l):
        tv.move(k, '', index)
    tv.heading(col,
               command=lambda: treeview_sort_column2(tv, col, not reverse))
...
for col in columns:
    if (col == 'a') or (col == 'e'):  #数字排序
        tree.heading(col,
                     text=col,
                     command=lambda _col=col: treeview_sort_column2(
                         tree, _col, False))  #重建标题，添加控件排序方法
    else:  #默认排序
        tree.heading(
            col,
            text=col,
            command=lambda _col=col: treeview_sort_column1(tree, _col, False))
...
Button(frm_B1, text="推荐相关", bg='gold', command=recommend1).grid(row=1,
                                                                column=0,
                                                                sticky=W)
var5 = IntVar()
c01 = Checkbutton(frm_B1, text='禁止文件夹\n穿透(仅支持\n特征码识图)',
                  variable=var5).grid(row=2, column=0, rowspan=2, sticky=W)

Label(frm_B1, text='算法功能选择', font=ft1).grid(row=0, column=1, sticky=W)  #下拉列表
number = StringVar()
numberChosen = ttk.Combobox(frm_B1,
                            width=16,
                            textvariable=number,
                            state='readonly')
numberChosen['values'] = ('特征码识图（纹理）', '直方图识图（灰度明暗）', '主色调识图（色调）',
                          '文件名相似度（文件名）', '小图找全图（SIFT特征）', '画图识图（颜色位置）',
                          '相似人脸识图')  # 设置下拉列表的值
numberChosen.grid(column=1, row=1)  # 设置其在界面中出现的位置  column代表列   row 代表行
numberChosen.current(0)  # 设置下拉列表默认显示的值，0为 numberChosen['values'] 的下标值
numberChosen.grid(row=1, column=1, sticky=W)

Label(frm_B1, text='图像甄别与其他功能', font=ft1).grid(row=0, column=2,
                                               sticky=W)  #下拉列表
Button(frm_B1, text="找出路径内所有黑白图片", bg='yellow',
       command=SearchBW).grid(row=1, column=2, sticky=W)
Button(frm_B1, text="查路径内所有重复图片（纹理）", bg='yellow',
       command=SearchRepeat).grid(row=2, column=2, sticky=W)
Button(frm_B1, text="识别图片英文", bg='green', command=readEnglish).grid(row=3,
                                                                    column=2,
                                                                    sticky=W)
...
#var6.set("输入标签试试看")
Button(frm_B1, text="查看所选单张图片敏感程度", bg='yellow', command=Isporn).grid(row=1,
                                                                      column=3,
                                                                      sticky=W)
Button(frm_B1, text="扫描路径内所有人脸", bg='yellow',
       command=SearchFace).grid(row=2, column=3, sticky=W)
Button(frm_B1, text="保存所有结果至搜索路径下‘saveP’文件夹", bg='red',
       command=saveP).grid(row=3, column=3, sticky=W)

#筛选
Label(frm_B1, text='文件筛选', font=ft1).grid(row=0, column=4, sticky=W)  #范围:时间、大小
var4_1 = IntVar()
c11 = Checkbutton(frm_B1, text='go！自动执行筛选', variable=var4_1).grid(row=1,
                                                                  column=4,
                                                                  sticky=W)
Button(frm_B1, text="开始筛选上方表格", bg='yellow', command=sieve).grid(row=2,
                                                                 column=4,
                                                                 sticky=E)
...
var7 = StringVar()
en1 = Entry(frm_B1, width=22, textvariable=var7).grid(row=1,
                                                      column=6,
                                                      sticky=E)
var7.set("100")
var8_1 = StringVar()
...
B0 = Button(frm_B2,
            ...
            width=8,
            height=3,
            command=lambda: recommend2(path2=var2.get()))
